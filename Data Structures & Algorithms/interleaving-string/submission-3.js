class Solution {
    ...
    isInterleave(s1, s2, s3) {
        ...
        //        cache.set(key, res1);
        //        return res1;
        //        //equals to s2 but not s1
        //      }else if(s2[j]===s3[k]&&s1[i]!==s3[k]){
        //         let res2 = recursion(i,j+1,k+1);
        //         cache.set(key, res2);
        //         return res2;
        //         //equals to s2 and s1 or take which one return true that one:
        ...
        for(let r = R-1; r>=0; r--){
            for(let c=C-1;c>=0;c--){
              if(r < s1.length && s1[r]===s3[r+c] && dp[r+1][c]){
                dp[r][c] = true;
              }
              if(c<s2.length && s2[c]===s3[r+c] && dp[r][c+1]){
                dp[r][c] = true;
              }
            }
        }
